...
func TestHandler(t *testing.T) {
	...
	Convey("Handler returns matching greeting from request page model", t, func() {
		recorder := httptest.NewRecorder()
		rdr := bytes.NewReader([]byte(`{"greeting": "Foo bar"}`))
		request, err := http.NewRequest("POST", "/", rdr)
		So(err, ShouldBeNil)
		request.Header.Set("Accept-Language", "en")
		Handler(recorder, request)
		So(recorder.Code, ShouldEqual, 200)
		So(f.binding, ShouldHaveSameTypeAs, &hello.Page{})
		p := f.binding.(*hello.Page)
		So(p.Greeting, ShouldEqual, "Foo bar")
